...
TEST_CASE("ex.pals.yaml has expected structure", "[ex.pals.yaml][structure]") {
    YAMLNodeHandle root = parse_file("../lattice_files/ex.pals.yaml");
    REQUIRE(root != nullptr);

    SECTION("Root is a sequence") {
        REQUIRE(is_sequence(root));
        REQUIRE(size(root) >= 2);
    }

    SECTION("First element has thingB") {
        YAMLNodeHandle first = get_index(root, 0);
        REQUIRE(first != nullptr);
        REQUIRE(is_map(first));
        REQUIRE(has_key(first, "thingB"));

        YAMLNodeHandle thingB = get_key(first, "thingB");
        REQUIRE(is_map(thingB));
        REQUIRE(has_key(thingB, "kind"));

        YAMLNodeHandle kind = get_key(thingB, "kind");
        char* kind_str = as_string(kind);
    ...
    SECTION("Second element has inj_line") {
        YAMLNodeHandle second = get_index(root, 1);
        REQUIRE(second != nullptr);
        REQUIRE(is_map(second));
        REQUIRE(has_key(second, "inj_line"));

        YAMLNodeHandle inj_line = get_key(second, "inj_line");
        REQUIRE(is_map(inj_line));
        REQUIRE(has_key(inj_line, "kind"));
        REQUIRE(has_key(inj_line, "multipass"));
        REQUIRE(has_key(inj_line, "line"));

        YAMLNodeHandle kind = get_key(inj_line, "kind");
        char* kind_str = as_string(kind);
        REQUIRE(std::string(kind_str) == "BeamLine");

        YAMLNodeHandle multipass = get_key(inj_line, "multipass");
        REQUIRE(as_bool(multipass) == true);

        yaml_free_string(kind_str);
        delete_node(kind);
        delete_node(multipass);
        delete_node(inj_line);
        delete_node(second);
    }
    ...
    SECTION("inj_line has line sequence") {
        YAMLNodeHandle second = get_index(root, 1);
        YAMLNodeHandle inj_line = get_key(second, "inj_line");
        YAMLNodeHandle line = get_key(inj_line, "line");

        REQUIRE(is_sequence(line));
        REQUIRE(size(line) >= 3);
